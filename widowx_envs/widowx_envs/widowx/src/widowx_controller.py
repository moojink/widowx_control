...
MAX_EFFORT_ABS_VALUES = np.array([800, 1000, 700.0, 600.0, 600.0, 700.0])
...
class WidowX_Controller(RobotController):
    ...
    def move_to_eep(self, target_pose, duration=1.5, blocking=True, check_effort=True):
        try:
            if not blocking:
                ...
                solution, success = self.bot.arm.set_ee_pose_matrix(target_pose, custom_guess=self.get_joint_angles(), moving_time=duration, accel_time=duration * 0.45)
    ...
    def move_to_starteep(self, target_pose, duration=1.5, blocking=True, check_effort=True):
        ...
        for step in range(substeps):
            des_joint = (step / substeps) * np.array(solution) + (1 - step / substeps) * start_joint_angles
            self.bot.arm.publish_positions(des_joint, moving_time=duration / 100)
    ...
    def move_to_neutral(self, duration=4):
        ...
        try:
            ...
            start_joint_angles = self.get_joint_angles()
            for step in range(substeps):
                des_joint = (step/substeps) * self.neutral_joint_angles + (1 - step/substeps) * start_joint_angles
                self.bot.arm.publish_positions(des_joint, moving_time=duration/100)
                ...
                if np.max(np.abs(self.get_joint_effort()) - MAX_EFFORT_ABS_VALUES) > 10:
                    ...
                    print('wiggling before reattempting move to neutral..')
                    for i in range(10):
                        des_joint = des_joint + np.random.normal(np.zeros(6), np.ones(6) * 0.015)
                        self.bot.arm.publish_positions(des_joint, moving_time=0.3)
                    # self.move_to_neutral()
                    # rospy.sleep(1.0)
                    # self.move_to_neutral()
