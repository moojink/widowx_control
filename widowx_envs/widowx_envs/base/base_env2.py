...
class BaseRobotEnv2(BaseEnv):
    def __init__(self, env_params):
        ...
        self.controller = self._hp.robot_controller(self._robot_name, self._hp.print_debug, gripper_attached=self._hp.gripper_attached, gripper_params=self._hp.gripper_params, normal_base_angle=self._hp.workspace_rotation_angle_z)
        ...
        if not self._hp.start_at_current_pos:
            self.controller.move_to_neutral(duration=3)
    ...
    def _next_qpos(self, action):
        ...
        delta_transform, grasp_action = tr.action2transform_local(action, self.controller.get_cartesian_pose()[:3])
    ...
    def get_target_state(self):
        return tr.state2transform(self._previous_target_qpos, self.controller.default_rot)
    ...
    def step(self, action, tstamp_return_obs=None, blocking=True):
        ...
        if self._hp.continuous_gripper:
            self.controller.set_continuous_gripper_position(new_gripperstate)
        else:
        ...
        self._previous_target_qpos = tr.transform2state(new_transform, new_gripperstate, self.controller.default_rot)
    ...
    def get_full_state(self):
        eep = self.controller.get_cartesian_pose(matrix=True)
        return tr.transform2state(eep, self.controller.get_gripper_position(), self.controller.default_rot)

    def _get_obs(self):
        obs = {}
        t0 = time.time()
        j_angles, j_vel, eep = self.controller.get_state()
        obs['joint_effort'] = self.controller.get_joint_effort()
        ...
        obs['eef_transform'] = self.controller.get_cartesian_pose(matrix=True)
    ...
    def _move_to_state(self, target_xyz, target_zangle, duration=1.5):
        self.controller.move_to_state(target_xyz, target_zangle, duration=duration)
    ...
    def move_to_neutral(self, duration=2.):
        self.controller.move_to_neutral(duration)
        self._reset_previous_qpos()
